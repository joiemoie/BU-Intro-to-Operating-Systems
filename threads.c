...
void pthread_exit(void *value_ptr)
{
	if(TCBarray[spot].threadWaiting!=0) TCBarray[TCBarray[spot].threadWaiting].state = READY;
...
int sem_init(sem_t *sem, int pshared, unsigned value) {
	struct semExtension * newSemExtension = (struct semExtension*)malloc(sizeof(struct semExtension));
	newSemExtension->isInitialized = 1;
	newSemExtension->currentValue = value;
	struct queue * waitingThreads = (struct queue*)malloc(sizeof(struct queue));
	waitingThreads->head = (pthread_t *)malloc(sizeof(pthread_t) * 129);
	waitingThreads->tail = waitingThreads->head;
	waitingThreads->originalHead = waitingThreads->head;
...
int sem_wait(sem_t *sem) {
	...
	if (newSemExtension->currentValue <= 0) {
		*(newSemExtension->waitingThreads->tail) = spot;
		(newSemExtension->waitingThreads->tail)++;
		TCBarray[spot].state = BLOCKED;
		schedule();
	}
...
int sem_post(sem_t *sem) {
	...
	if (newSemExtension->currentValue > 0) {
		...
		(newSemExtension->waitingThreads->head)++;
	}
}
int sem_destroy(sem_t *sem) {
	struct semExtension * newSemExtension = (struct semExtension *)(sem->__align);
	if (newSemExtension->isInitialized == 1) {
		free(newSemExtension->waitingThreads->originalHead);
